...
public class RenewUserSubscriptionConsumer {
    private final static String CACHE_KEY = "user-subscription:renew:message:";
    ...
    @KafkaListener(
        topics = "${spring.kafka.topics.user-subscription-renew}",
        groupId = "${spring.kafka.consumer.group-id}",
        containerFactory = "topicWithDltFixedRetryContainerFactory"
    )
    public void execute(String message) {
        try {
            AsyncMessageInput<AsyncRenewSubscriptionPlanInput> input = convertBody(message);

            log.info("[RenewUserSubscriptionConsumer#execute] - New message on consumer: {}", message);

            if (isNull(input) || isEmpty(input.getMessageHash()) || isNull(input.getData())) {
                log.error("[RenewUserSubscriptionConsumer#execute] - Message with invalid data will be discarded: {}", message);
                return;
            }
            ...
            if (cacheDataProviderPort.existsByKey(cacheKey)) {
                log.warn("[RenewUserSubscriptionConsumer#execute] - Message already processed: {}", message);
                return;
            }

            AsyncRenewSubscriptionPlanInput inputData = input.getData();
            ...
            log.error("[RenewUserSubscriptionConsumer#execute] - Error while consuming message: {}", message, e);
            throw e;
        }
    }

    private AsyncMessageInput<AsyncRenewSubscriptionPlanInput> convertBody(String message) {
        try {
            return mapper.readValue(message,
                new TypeReference<AsyncMessageInput<AsyncRenewSubscriptionPlanInput>>() {
                }
            );
        } catch (JsonProcessingException e) {
            log.error("[RenewUserSubscriptionConsumer#convertBody] - Error while converting message: {}", e.getMessage());
            return null;
        }
    }

    private void sendUserSubscriptionUpdateMessage(AsyncRenewSubscriptionPlanInput input) {
