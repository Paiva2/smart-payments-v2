...
public class CancelSubscriptionPlansJob {
    ...
    @Scheduled(cron = "0 10 0 * * *") // Runs everyday at midnight (00:10)
    public void run() {
        List<UserSubscription> subscriptionsList = userSubscriptionDataProviderPort.findAllMonthlyToCancel();

        log.info("[CancelSubscriptionPlansJob#run - Starting subscription cancellations]");

        subscriptionsList.forEach(this::sendMessageToCancel);

        log.info("[CancelSubscriptionPlansJob#run - End subscription cancellations. Total: {}", subscriptionsList.size());
    }

    private void sendMessageToCancel(UserSubscription userSubscription) {
        try {
            kafkaTemplate.send(userSubscriptionStatesTopic, mapper.writeValueAsString(input(userSubscription)));
        } catch (Exception e) {
            log.info("[CancelSubscriptionPlansJob#sendMessageToCancel - Error while sending message to cancel subscription. Error: {}", e.getMessage());
        }
    }
    ...
    private AsyncMessageOutput<Object> input(UserSubscription userSubscription) {
        ...
        final AsyncSubscriptionPlanStateInput renewSubscriptionInput = AsyncSubscriptionPlanStateInput.builder()
            .state(EUserSubscriptionState.CANCELLED)
