drop view vi_user_subscription_resume;

create view vi_user_subscription_resume as
select usr.id as user_id,
       ...
       coalesce(SUM(usbch.amount) FILTER (
           WHERE usbch.credit_type = 'EMAIL' AND usbch.expires_at IS NULL
               AND (usbch.valid_from <= now() OR usbch.valid_from IS NULL)
           ), 0) AS email_credits,
       coalesce(SUM(usbch.amount) FILTER (
           WHERE usbch.credit_type = 'EMAIL' AND usbch.expires_at IS NOT NULL
               AND usbch.expires_at >= now()
               AND (usbch.valid_from <= now() OR usbch.valid_from IS NULL)
           ), 0) AS subscription_email_credits,


       coalesce(SUM(usbch.amount) FILTER (
           WHERE usbch.credit_type = 'SMS' AND usbch.expires_at IS NULL
               AND (usbch.valid_from <= now() OR usbch.valid_from IS NULL)
           ), 0) AS sms_credits,
       coalesce(SUM(usbch.amount) FILTER (
           WHERE usbch.credit_type = 'SMS' AND usbch.expires_at IS NOT NULL
               AND usbch.expires_at >= now()
               AND (usbch.valid_from <= now() OR usbch.valid_from IS NULL)
           ), 0) AS subscription_sms_credits,

       coalesce(SUM(usbch.amount) FILTER (
           WHERE usbch.credit_type = 'WHATS_APP' AND usbch.expires_at IS NULL
               AND (usbch.valid_from <= now() OR usbch.valid_from IS NULL)
           ), 0) AS whatsapp_credits,
       coalesce(SUM(usbch.amount) FILTER (
        WHERE usbch.credit_type = 'WHATS_APP' AND usbch.expires_at IS NOT NULL
            AND usbch.expires_at >= now()
            AND (usbch.valid_from <= now() OR usbch.valid_from IS NULL)
        ), 0) AS subscription_whatsapp_credits
from users usr
